...
public final class SparseEncodingProcessor extends InferenceProcessor {
    ...
    public void doExecute(
        ...
        if (skipExisting == false) {
            makeInferenceCallWithMapResult(ingestDocument, processMap, inferenceList, pruneType, pruneRatio, handler);
            return;
        }
        ...
        openSearchClient.execute(GetAction.INSTANCE, new GetRequest(index, id), ActionListener.wrap(response -> {
            ...
            if (filteredInferenceList.isEmpty()) {
                ...
                makeInferenceCallWithMapResult(ingestDocument, filteredProcessMap, filteredInferenceList, pruneType, pruneRatio, handler);
            }
